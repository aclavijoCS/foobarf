...
export class FooBarAlg {
  ...
  async choice(r: Robot): Promise<any> {
    ...
    if (
      this.money >= 3 &&
      this.materialsContainer.foo.length > 6 &&
    ...
    // sell foobar
    if (
      this.materialsContainer.foobar.length >= 3 &&
      this.generalStatus.isRobotSelling < this.generalStatus.numberSellers
    ) {
      this.generalStatus.isRobotSelling += 1;
  ...
  loggingInfo(): void {
    console.clear();
    console.log(`
            [Robots]: ${this.robotContainer.length}, [Money]: ${
      this.money
    }, [Nb Sellers Max]: ${
